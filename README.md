...
| **내보내기** | 마크다운 · 기능명세서 CSV · IA CSV · 플랜 JSON · **UniAI UI 생성(디자인 스킬 토큰 반영)** · 코딩 에이전트 번들 · Mermaid |
...
| **계정** | 이메일·비밀번호 로그인. 플랜이 계정에 저장되어 다른 기기에서 이어서 볼 수 있습니다 |
...
정사각형 이미지를 권장하며, 파일이 없으면 `UB` 이니셜 마크로 대체됩니다.
...
# DeepSeek 출력 상한은 앱 전체에서 384000 토큰으로 고정됩니다.
```
...
src/
  app/
    ...
    api/
      ...
      auth/                      로그인 · 가입 · 로그아웃 · 현재 사용자
      plans/                     계정별 플랜 저장소
  lib/
    types.ts                     도메인 모델 (단일 계약)
    store.ts                     zustand + localStorage 영속화
    sync.ts                      플랜을 서버와 맞춤 (브라우저 우선, 뒤에서 올림)
...
- **브라우저가 정본이고 계정은 그 뒤를 따라갑니다.** 편집은 예전처럼 localStorage 에 즉시
  저장되고, 1.2초 뒤 계정으로 올라갑니다. 인터넷이 끊겨도 작업이 멈추지 않고, 올리기에
  실패해도 내용이 사라지지 않습니다. 같은 플랜이 양쪽에 있으면 나중에 고친 쪽이 남습니다.
- **한 SQL 로 두 엔진을 씁니다.** 배포는 Postgres(`DATABASE_URL`), 로컬은 PGlite —
  Postgres 를 WebAssembly 로 빌드한 것입니다. 둘 다 진짜 Postgres 라 데이터베이스를
  설치하지 않고도 같은 질의를 실제로 실행해 검증할 수 있습니다.
- **API 키는 서버에만 있습니다.** `/api/generate` 와 `/api/chat` 은 로그인해야
  부를 수 있습니다 — 주인의 키로 도는 요청이기 때문입니다.
