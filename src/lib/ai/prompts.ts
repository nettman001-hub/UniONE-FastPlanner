...
export function buildChatPrompt(plan: Plan, question: string): string {
  return [
    ...
    '문서 전체를 다시 적을 필요가 없습니다.',
    '항목은 **ID 로 맞춥니다.** 고치는 항목에는 위 "현재 문서" 의 ID 를 그대로 적고,',
    '새로 만드는 항목에는 id 를 비워 두세요. ID 는 시스템이 붙입니다.',
    '**무엇도 지울 수 없습니다.** 삭제 요청이면 patch.artifact 를 none 으로 두고',
    '해당 화면에서 직접 지워 달라고 답하세요.',
    '',
    '### 산출물별 형태',
    '- `fs` — 고칠 것이 상세 기능뿐이면 specifications 만 담습니다.',
    '  새 기능에는 상위 requirementId, 새 상세 기능에는 상위 featureId 를 반드시 적으세요.',
    '  예: `{"specifications":[{"featureId":"FN-001","title":"...","actor":"...",' +
      '"precondition":"...","mainFlow":[],"exceptions":[],"acceptanceCriteria":[],"priority":"P1"}]}`',
    '- `prd` — **고칠 칸만** 담습니다. 개요만 고치면 `{"overview":"..."}` 하나면 됩니다.',
    '  담은 칸은 통째로 바뀌므로, 목록을 고칠 때는 그 목록의 항목을 빠짐없이 적으세요.',
    '- `ia` — 화면 단위입니다. 상위 화면은 parentId(PG-형식)로 가리키고, 최상위는 null 입니다.',
    '  예: `{"pages":[{"id":"PG-003","name":"새 이름","featureIds":["FN-002"]}]}`',
    '- `flow` — **플로우 하나가 단위**입니다. 고치는 플로우에는 id(FL-형식)를 적고',
    '  nodes 와 edges 를 그 플로우 기준으로 전부 담으세요 (단계는 서로 얽혀 있습니다).',
    '  이름·설명만 고칠 때는 nodes 를 생략하면 그림이 그대로 남습니다.',
    '- `wireframe` — 화면 하나가 단위입니다. pageId 로 맞추며 그 화면의 blocks 를 전부 담습니다.',
    '  다른 화면의 그림은 건드리지 않습니다.',
    '',
    '한 번에 다루기 어려울 만큼 요청이 크면, 처리할 수 있는 만큼만 담고',
    'reply 에 무엇이 남았는지 적어 주세요. 억지로 다 담으려다 빠뜨리지 마세요.',
    '',
    '## 서비스 정보',
    briefBlock(plan.brief),
    '',
