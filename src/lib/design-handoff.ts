 ...
 *
 * ## 왜 "자동 연동" 이 아니라 이런 모양인가
 *
 * 구글 스티치·Figma 같은 도구에는 **밖에서 화면을 만들어 넣는 공개 API 가 없다.**
 * Figma REST 는 읽기 위주라 프레임을 만들지 못하고(그건 플러그인 영역이다),
 * 스티치는 프롬프트를 받아 도구 안에서 만든다. 그래서 "누르면 저쪽에 디자인이
 * 생긴다" 는 약속은 지금 아무도 지킬 수 없다.
 *
