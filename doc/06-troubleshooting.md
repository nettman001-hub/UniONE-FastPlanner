...
| 새로고침 | 계속 진행. 화면을 다시 그리면서 진행 상태를 되찾습니다 |
| 탭을 닫았다가 다시 열기 | 만들던 단계까지 마치고 **멈춰서 보관**합니다. 돌아오면 이어서 만들 수 있습니다 |
...
작업이 서버에서 사라진 경우입니다. 화면에 안내가 뜨고 **크레딧은 돌려드립니다.**

- **서버를 재시작**했다 — 기본 설정에서는 진행 중이던 작업이 메모리에만 있어 사라집니다.
  재시작에도 견디게 하려면 `.env.local` 에 `JOB_STORE_DIR=.jobs` 를 넣으세요.
- **30분이 지났다** — 끝난 작업은 30분간만 보관합니다.
- **서버리스(Vercel)에 올렸다** — 인스턴스가 여러 개로 흩어지면 결과를 못 찾을 수 있습니다.
  자세한 내용은 [7. 배포](./07-deploy.md#생성-작업-큐) 를 보세요.
