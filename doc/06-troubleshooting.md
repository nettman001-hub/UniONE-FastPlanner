...
정상입니다. 플랜은 계정에 있고 브라우저의 사본만 지운 것입니다.
**같은 계정으로 다시 로그인하면 그대로 돌아옵니다.**

한 컴퓨터를 여러 사람이 쓸 때 앞사람 플랜이 뒷사람에게 보이거나 뒷사람 계정으로
올라가는 것을 막기 위한 동작입니다.

### 만든 플랜이 사라졌음

먼저 **어느 계정으로 로그인했는지** 확인하세요. 오른쪽 위 계정 버튼에 이메일이
나옵니다. 다른 계정으로 들어오면 그 계정의 플랜만 보입니다.

로그인 기능이 생기기 **전에** 만든 플랜이라면, 처음 로그인할 때 화면 위에
`로그인 전에 이 브라우저에서 만든 플랜이 N개 있습니다` 안내가 뜹니다.
**내 계정으로 가져오기** 를 누르면 올라옵니다. 안내가 안 보이면 이미 옮겼거나
`버리기` 를 누른 것입니다.

배포한 서버에 `DATABASE_URL` 이 없으면 플랜이 계정에 저장되지 않습니다.
`/api/auth/me` 를 열어 `"database":"local"` 이면 그 경우입니다 —
[7. 배포](./07-deploy.md) 의 데이터베이스 절을 보세요.
...
계정으로 올리지 못하고 있다는 뜻입니다. **작업한 내용은 브라우저에 남아 있으니
당장 잃어버리지는 않습니다.**

1. 인터넷 연결을 확인합니다.
2. 새로고침해 봅니다. 로그인이 풀렸다면 로그인 화면으로 갑니다.
3. 그래도 그대로면 배포한 사람에게 `DATABASE_URL` 을 확인해 달라고 하세요.
   Supabase 무료 플랜은 일주일 넘게 안 쓰면 프로젝트가 일시 정지됩니다 —
   대시보드에서 `Restore` 를 누르면 다시 삽니다.

### 다른 기기로 옮기고 싶음

같은 계정으로 로그인하면 따라옵니다. 따로 할 일이 없습니다.
...
**Redeploy** 하세요. 자세한 내용은 [7. 배포](./07-deploy.md).
