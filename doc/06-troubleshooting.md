...
### 생성 중에 다른 메뉴로 넘어가도 되나요?

**됩니다.** 생성은 화면과 무관하게 계속 진행됩니다.

- 사이드바의 해당 메뉴에 **회전 표시**가 뜹니다.
- 옮겨 간 화면에는 "**… 생성 중입니다**" 안내가 나옵니다.
- 다 만들어지면 보고 있는 화면에 바로 반영됩니다.
- 진행 중에는 생성 버튼이 잠깁니다. 중복 실행과 이중 과금을 막기 위해서입니다.

**전체 자동 생성**도 마찬가지로 끝까지 돕니다.

다만 **탭을 닫거나 새로고침하면 중단됩니다.** 문서는 브라우저에서 만들어 보내는 구조라,
페이지가 사라지면 응답을 받을 곳이 없어집니다. 이 경우 크레딧은 이미 차감된 상태이므로
해당 단계를 다시 생성해야 합니다.
