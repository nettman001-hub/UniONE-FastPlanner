...
# DeepSeek 출력 상한: 앱 전체 384000 토큰 고정
```
...
**메모리가 아니라 파일입니다 — 껐다 켜도 그대로 있습니다.** 폴더를 옮기고 싶으면
`PGLITE_DIR` 로 정하세요.

```
PGLITE_DIR=C:/dev/uniboard-data
```

> 경로 구분자는 `/` 로 쓰세요. `\` 는 `.env` 파일에서 탈출 문자로 읽힐 수 있습니다.
>
> 저장소 폴더 **밖에** 두면 `git clean` 을 하거나 저장소를 다시 받아도 데이터가 남습니다.

⚠️ **배포에는 반드시 `DATABASE_URL` 을 넣으세요.** 배포 환경에서는 파일이
인스턴스와 함께 사라져, 다시 배포할 때마다 계정과 플랜이 전부 없어집니다.
관리자 화면의 **점검** 탭이 이 경우를 잡아 경고합니다.
