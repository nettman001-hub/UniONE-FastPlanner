...
`src/middleware.ts` 가 `/docs` 아래 전부를 막습니다. 세션이 없으면
`/login?next=<보려던 문서>` 로 보내고, 로그인하면 그 문서로 돌아옵니다.

다른 화면처럼 `RequireAuth`(브라우저)로 막지 **않은** 이유가 있습니다.

| | 내용이 어디서 오나 | 브라우저에서 가리면 |
| --- | --- | --- |
| 플랜·설정 화면 | API 로 따로 받아 옴 (서버에서 걸림) | 충분함 — 진짜 자물쇠는 API 에 있음 |
| 설명서 | **HTML 안에 통째로 실려 나감** | 소스에 그대로 남아 주소만 알면 읽힘 |

미들웨어를 걷어내고 로그인 없이 `/docs/start` 를 불러 보면 본문 5만 자가
그대로 응답에 실립니다. 그래서 **그리기 전에** 막습니다.

레이아웃에서 막지 않은 이유는 레이아웃이 **지금 주소를 모르기** 때문입니다.
그러면 로그인한 뒤 보던 문서로 되돌려 줄 수 없습니다. 미들웨어는 주소를 알고,
**빌드 때 미리 만들어 둔 정적 문서에도 그대로 걸립니다** — 정적 생성을 포기하지
않고 막을 수 있는 것이 이 방식뿐입니다.

세션 확인은 `lib/auth/session.ts` 를 그대로 씁니다. 거기서 `node:crypto` 대신
Web Crypto 만 쓰기로 해 둔 덕분에 Edge 런타임에서도 같은 코드가 돕니다.
`AUTH_SECRET` 이 없어 확인이 터지는 경우에는 **막는 쪽으로** 넘어갑니다.
...
> 키 이름이 서비스명(UniBoard)과 다른 것은 **의도한 것이다.** 사용자 브라우저에는
> 이미 옛 이름으로 저장되어 있어서, 이름을 맞추려고 키를 바꾸면 앱이 그 값을 찾지
> 못한다 — 화면에서는 만들어 둔 플랜이 전부 사라진 것으로 보인다.
> 정리하려면 옛 키를 읽어 새 키로 옮기는 이사 코드를 먼저 넣어야 한다.
> **표기와 저장소 키는 별개다.**
